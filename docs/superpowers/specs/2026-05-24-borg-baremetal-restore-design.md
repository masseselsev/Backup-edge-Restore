...
2. **`fastapi-backend`**:
   ...
   - Manages persistence using SQLite via SQLAlchemy and Alembic migrations.
3. **`celery-worker`**:
...
- Rewrites `/etc/fstab` with labels.
...
- Performs pruning locally or remotely.
...
- **Partition & Format**:
  ...
  - Formats ESP (`mkfs.vfat -n edgeboot`) and Root (`mkfs.ext4 -L edgeroot`).
- **Data Extract**: Mounts partitions to `/mnt/target` and runs local `borg extract`.
- **Network Injection**: Identifies and rewrites interface configurations in `/mnt/target/etc/network/interfaces` or Netplan to map MAC addresses or custom rules, preventing interface PCIe drift.
- **Bootloader**: Bind-mounts `/dev`, `/proc`, `/sys` and executes `grub-install` + `update-grub` in chroot. Copies standard EFI fallback path.
- **Verification Audit**: Scans filesystem structure and labels before unmounting.
...
4. **Input Validation**: All incoming endpoints are strictly validated using Pydantic models. Shell arguments for execution are parameterized.
